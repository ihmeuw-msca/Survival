...
def cumulative_survival(base_excess_mortality: float,
        ...
        for i in range(1,year):
            value *= 1 - P_c(base_excess_mortality, slope, i) - other_mortality
...
def P_c(base_excess_mortality: float,
        ...
        """
        value = base_excess_mortality*math.exp(slope*year)
...
class DecayMIRModel:
    ...
    def __init__(self,
        ...
        self.disease_period = np.asarray(disease_period)
    ...
    def get_survival_rate(self,
        ...
        for i in range(self.num_points):
            abs_survival_rate[i] = cumulative_survival(self.base_excess_mortality[i], 
                                                       self.slope[i], num_years, 
                                                       self.other_mortality[i])
            # Net survival.
            for t in range(1, num_years):
                rel_survival_rate[i] *= (1 - P_c(self.base_excess_mortality[i], self.slope[i], t))
    ...
    def mir_equation(base_excess_mortality: float,
        ...
        for year in range(1, disease_period):
            cum_survival = cumulative_survival(base_excess_mortality, slope, year, other_mortality)
            val_true += P_c(base_excess_mortality, slope, year)*cum_survival
